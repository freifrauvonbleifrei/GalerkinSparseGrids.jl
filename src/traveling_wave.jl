# -----------------------------------------------------------
#
# An example of constructing the initial data for a traveling
...
function cos_coeffs(k::Int, n::Int, m::AbstractArray{T,1};
    ...
    sine_dicts   = [coeffs_DG(1, k, n, sines[i], scheme=scheme) for i in 1:D]
    cosine_dicts = [coeffs_DG(1, k, n, cosines[i], scheme=scheme) for i in 1:D]
...
function cos_coeffs(k::Int, n::Vector{Int}, m::AbstractArray{T,1};
                    scheme="sparse", phase = 0.0, A = 1.0) where T
    D = length(m)
    wavenumber = 2*pi*m
    ...
    sine_dicts   = [coeffs_DG(1, k, n[i], sines[i], scheme=scheme) for i in 1:D]
    cosine_dicts = [coeffs_DG(1, k, n[i], cosines[i], scheme=scheme) for i in 1:D]
...
function sin_coeffs(k::Int, n::Vector{Int}, m::Array{Int,1};
    scheme="sparse", phase=0.0, A=1.0)
    return cos_coeffs(k, n, m; scheme=scheme, phase=phase-pi/2, A=A)
end
# -----------------------------------------------------
...
function traveling_wave(k::Int, n::Vector{Int}, m::Array{Int,1};
                        scheme="sparse", phase=0.0, A=1.0)
    wavenumber = 2*pi*m
    frequency = sqrt(dot(wavenumber,wavenumber))
